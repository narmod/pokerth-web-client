...
- Fully **localized in all 51 languages** and switchable instantly, with no reload
...
- **Full i18n in 51 languages**, switchable on the fly and auto-detected from the browser locale — the complete official PokerTH language set plus community additions (Ukrainian, Romanian, Croatian, Serbian and more), with Brazilian and European Portuguese shipped as separate catalogues (pt-BR / pt-PT)
...
- **In-app help** — a chaptered user guide built into the client (getting started, the lobby, the table, options, troubleshooting…), with its own catalogue in each of the 51 languages
...
- **Startup loading screen** matching the login look (theme-aware, 51 languages) that preloads the critical assets, retries on a flaky connection, and offers a **Retry** button instead of silently loading a broken UI
...
│   ├── modules/             # ES modules
│   │   ├── i18n.mjs                 internationalisation (51 languages)
