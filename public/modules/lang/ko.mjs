...
export const strings = {
  ...
  sectionSeat: '좌석',    botDifficulty:'봇 난이도', botEasy:'쉬움', botMixed:'혼합', botNormal:'보통', botHard:'어려움',
    ...
    pvUpdated:'마지막 업데이트: 2026년 7월 17일',
    pvWhoH:'운영자 안내',
    ...
    pvLocalH:'브라우저에 남는 것',
    pvLocal:'설정(테마, 언어, 테이블 스타일, 소리 옵션 등)은 브라우저에 로컬로 저장되며 기기를 떠나지 않습니다. 브라우저 설정에서 언제든지 지울 수 있습니다.',
    pvServerH:'서버가 보는 것',
    pvSrv1:'익명 방문 카운터: 브라우저에 저장된 무작위 식별자로 서로 다른 기기와 방문 수를 셉니다. 이 통계에는 이름도 IP 주소도 포함되지 않으며 약 13개월 후 삭제됩니다.',
    pvSrv2:'기술적 연결 데이터: 모든 웹 서버와 마찬가지로 서비스 제공과 보호에 필요한 연결 데이터(IP 주소 포함)를 받습니다. 다른 용도로는 사용하지 않습니다.',
    pvSrv3:'게임 데이터: 랭킹 서버에서 플레이하면 선택한 닉네임과 결과가 순위표 표시를 위해 기록됩니다.',
    pvThirdH:'제3자 서비스',
    pvThird:'pokerth.net(또는 다른 게임 서버)에 접속하면 이 웹 클라이언트와 독립적으로 운영되고 자체 규칙이 있는 서비스를 이용하는 것입니다. 이 페이지는 웹 클라이언트와 이를 호스팅하는 서버만 다룹니다.',
    pvThirdTr:'채팅 번역: 채팅 메시지의 번역 버튼(🌐)을 누르면 해당 메시지의 텍스트가 Google 번역 서비스(Google 번역)로 전송되어 사용자의 언어로 번역됩니다. 버튼을 누르지 않으면 아무것도 전송되지 않으며, 이 기능은 고급 옵션에서 끌 수 있습니다.',
