...
export const strings = {
  ...
  sectionSeat: '座席',    botDifficulty:'ボットの難易度', botEasy:'やさしい', botMixed:'ミックス', botNormal:'ふつう', botHard:'むずかしい',
    ...
    advIgnoredRemove:'削除', advLobbyChat:'ロビーチャットを表示', advBackGuard:'戻るボタンはゲームを終了せずウィンドウを閉じます（Android）', backAgainToExit:'終了するにはもう一度戻るを押してください', advBrowserZoom:'スマートフォン / タブレットでブラウザのズーム（ピンチ）を許可', advTableZoom:'テーブルズームを有効化（拡大鏡ボタン）', advChatTranslate:'チャット翻訳（Google 翻訳、メッセージごとのボタン）', chatTranslateUnsupported:'このブラウザは翻訳に対応していません（最新の Chrome/Edge が必要）。',
    ...
    pvSrv1:'訪問統計: ブラウザが生成したランダムな識別子で、端末数と訪問数を数えます。サーバーが保存するのはそのハッシュ値だけで、名前や IP アドレスと結び付けることはありません。日ごとの数値は約 13 か月後に削除されます。新規端末と再訪端末を区別するためのハッシュ化された識別子は、それより長く保持されます。ブラウザ、OS、言語は合計数としてのみ数えます。',
