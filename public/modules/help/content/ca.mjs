  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'famboard', t: 'Classificació familiar',
          b: ['Només en servidors privats i partides LAN, el client desa estadístiques acumulades per sobrenom — mans i partides jugades i guanyades, guany més gran, millor ratxa — i les comparteix mitjançant el servidor, de manera que tots els dispositius de la taula veuen la mateixa classificació. Les partides de pokerth.net no es registren mai així, i les estadístiques del mode entrenament es guarden completament a part.', 'En aquestes partides, el botó del trofeu obre la finestra de classificacions a la pestanya LAN: tots els jugadors, ordenables segons diversos criteris.'] },
        { id: 'language', t: 'Idioma',
          b: ['La interfície està disponible en 72 idiomes. Canvia\u2019l quan vulguis a les Opcions avançades (menú de l\u2019engranatge), categoria Interfície d\u2019usuari. Els termes d\u2019acció del pòquer (Fold, Check, Call, Bet, Raise, All-In) es mantenen en anglès per convenció, igual que al client d\u2019escriptori.'] },
        ...
        { id: 'avatar', t: 'Sobrenom i avatar',
          b: ['Tria el teu sobrenom i el teu avatar a la pantalla d\u2019inici de sessió abans de connectar-te. A pokerth.net, el teu sobrenom és el nom del teu compte; els avatars es comparteixen amb els altres jugadors mitjançant el servidor d\u2019avatars.', 'El teu avatar s\u2019envia en connectar-te i tots els jugadors veuen aquest mateix. Si el canvies mentre estàs connectat, el nou avatar s\u2019aplica a partir de la propera connexió. La inicial (Aa) no s\u2019envia: els altres jugadors veuen l\u2019avatar per defecte.'] }
      ]
    },
    {
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'zoom', t: 'Zoom de la taula (mòbils)',
          b: ['En pantalles petites, els botons de lupa amplien la taula (2\u00d7) i la pots arrossegar amb el dit — només la barra d\u2019accions es queda fixa; la teva caixa també s\u2019amplia i la vista hi torna quan és el teu torn. La vista segueix automàticament el seient actiu i s\u2019allunya al showdown per veure el conjunt. Es pot desactivar a les Opcions avançades. Al teu torn, si les cartes comunes queden fora de la vista, n\u2019apareix una petita còpia a la part superior de la taula; toca-la per anar a les cartes i tornar.'],
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'logopts', t: 'Opcions de registre',
          b: ['A Opcions avançades \u2192 Missatges de registre pots activar o desactivar el registre i triar l\u2019interval d\u2019escriptura, amb els mateixos tres ajustos que el client d\u2019escriptori: després de cada acció, després de cada mà (per defecte) o després de cada partida. Una altra opció escriu el fitxer .pdb en una carpeta que triïs i el manté al dia amb aquest interval, i una vegada més quan surts de la pàgina, perquè una altra eina pugui seguir la partida en directe.'],
          note: 'Escriure en una carpeta local necessita l\u2019API File System Access: només Chrome, Edge i Opera d\u2019escriptori. En la resta l\u2019opció s\u2019explica tota sola i l\u2019exportació manual des de la finestra de registres continua disponible. Un navegador només pot substituir un fitxer, mai afegir-hi, així que una eina que llegeixi el .pdb l\u2019hauria de tornar a obrir després de cada canvi.' },
        ...
        { id: 'hud', t: 'HUD d\u2019estadístiques als seients',
          b: ['El HUD adjunta una petita caixa d\u2019estadístiques al costat del seient de cada jugador, construïda a partir de les mans que has enregistrat als teus registres: nombre de mans observades, després VPIP (amb quina freqüència posa diners voluntàriament pre-flop), PFR (pujades pre-flop) i AF (factor d\u2019agressivitat), amb un codi de color del passiu a l\u2019agressiu. A sota, una insígnia resumeix el jugador en paraules \u2014 Tancat-Passiu, Obert-Agressiu, etc. \u2014 al costat d\u2019un petit quadrant on el sector encès es llegeix d\u2019esquerra a dreta de tancat a obert, i de baix a dalt de passiu a agressiu. La insígnia apareix des de la primera mà però queda atenuada fins a les 25 mans, llindar a partir del qual esdevé fiable. Toca una caixa per obtenir un popover detallat amb totes les xifres (3-bet, continuation bet, fold davant el 3-bet, intents de robatori, taxes de showdown\u2026), i arrossega-la si tapa alguna cosa.',
              'El HUD només sap el que has vist a les teves pròpies taules — llegeix els teus registres locals de mans, així que l\u2019enregistrament ha d\u2019estar activat i les xifres només tenen sentit després de prou mans. Està desactivada per defecte: activa-la a Opcions avançades \u2192 Assistència.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'music', t: 'Reproductor de música',
          b: ['L\u2019entrada de música dels menús de capçalera obre un petit reproductor de música ambiental: tria una pista de la llista, reprodueix/pausa, anterior/següent, aleatori, i repetició d\u2019una pista, de tota la llista o de res. El volum, la pista triada i el mode de repetició es recorden. La reproducció no comença mai sola — els navegadors exigeixen un toc — i el reproductor és totalment independent dels efectes de so del joc.', 'Dos polzes sota el títol de la pista diuen si t\u2019agrada el que sona. Un vot anònim per dispositiu, ràdios incloses, i el pots canviar o retirar en qualsevol moment; si l\u2019operador no mostra els totals, només veus el teu polze.', "A l’iPhone i l’iPad el reproductor fa servir per defecte una reproducció simple, perquè la música continuï amb CarPlay, Bluetooth o la pantalla bloquejada; el volum s’ajusta aleshores amb els botons del dispositiu o del cotxe. L’opció «Volum a l’app» recupera el control de volum, el balanç i el vúmetre, però el so pot entretallar-se al cotxe."] },
    ...
    },
    {
      ...
      sections: [
        { id: 'where', t: 'On viuen les opcions',
          b: ['Les Opcions avançades s\u2019obren des de l\u2019entrada de l\u2019engranatge de qualsevol menú de capçalera. Estan agrupades com al client d\u2019escriptori: Interfície d\u2019usuari, Estil, So, Partida local, Partida en xarxa, Partida per Internet, Sobrenoms / Avatars, Missatges de registre, i Còpia de seguretat i restabliment. Cada funció específica del web hi té el seu propi interruptor, així pots desactivar tot allò que no facis servir.'] },
        { id: 'cfgxml', t: 'Intercanviar paràmetres amb el client d\u2019escriptori',
          b: ['Els teus paràmetres poden viatjar entre clients: la categoria Còpia de seguretat i restabliment ofereix exportació/importació del fitxer oficial config.xml (aquell \u007e/.pokerth/config.xml que fan servir els clients d\u2019escriptori i QML). L\u2019exportació escriu els paràmetres compartits — nom, opcions de visualització, sons, preferències de taula, cegues, estils — i la importació aplica aquí un fitxer de l\u2019escriptori. Els paràmetres que aquest client no coneix es conserven intactes al fitxer.', 'Les teves notes de jugador també viatgen amb el fitxer: el text i la valoració amb estrelles, escrits tal com els llegeixen els clients d\u2019escriptori. Les etiquetes de color es queden en aquest client: el format oficial no hi té cap camp, així que una importació no les toca mai.'] },
