  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'famboard', t: 'Familien-Rangliste',
          b: ['Nur auf privaten Servern und in LAN-Spielen führt der Client Langzeitstatistiken pro Spitzname — gespielte und gewonnene Hände und Partien, größter Gewinn, beste Serie — und teilt sie über den Server, sodass jedes Gerät am Tisch dieselbe Rangliste sieht. pokerth.net-Spiele werden auf diese Weise nie erfasst, und die Statistiken des Trainingsmodus bleiben vollständig getrennt.', 'In diesen Spielen öffnet die Pokal-Schaltfläche das Ranglistenfenster auf dem Tab LAN: alle Spieler, sortierbar nach mehreren Kriterien.'] },
        { id: 'language', t: 'Sprache',
          b: ['Die Oberfläche ist in 72 Sprachen verfügbar. Ändere sie jederzeit in den Erweiterten Optionen (Zahnrad-Menü) unter Benutzeroberfläche. Die Poker-Aktionsbegriffe (Fold, Check, Call, Bet, Raise, All-In) bleiben per Konvention englisch, genau wie im Desktop-Client.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'zoom', t: 'Tisch-Zoom (Telefone)',
          b: ['Auf kleinen Bildschirmen zoomen Lupen-Schaltflächen den Tisch (2\u00d7) und du kannst mit dem Finger schwenken — nur die Aktionsleiste bleibt fest; deine eigene Box wird mitvergrößert, und die Ansicht kehrt zu ihr zurück, wenn du am Zug bist. Die Ansicht folgt automatisch dem aktiven Sitz und zoomt beim Showdown für die Übersicht wieder heraus. Abschaltbar in den Erweiterten Optionen. Bist du am Zug und die Gemeinschaftskarten liegen außerhalb der Ansicht, erscheint oben am Tisch eine kleine Kopie davon; tippe sie an, um zu den Karten und wieder zurück zu springen.'],
      ...
      sections: [
        ...
        { id: 'journal', t: 'Hand-Protokolle und das Logs-Fenster',
          b: ['Über das Live-Log hinaus wird jede Hand, die du spielst, lokal im Browser aufgezeichnet, im selben Format wie die .pdb-Protokolldateien des offiziellen Clients. Das Logs-Fenster (Erweiterte Optionen \u2192 Log-Meldungen \u2192 Logs verwalten\u2026) listet deine Sitzungen und lässt dich damit arbeiten: eine Sitzung mit Suche und Hervorhebung ansehen, nach Spiel filtern, als HTML oder Klartext exportieren, die rohe .pdb-Datei speichern oder eine vom Desktop-Client aufgezeichnete .pdb importieren. Sitzungen lassen sich einzeln oder alle auf einmal löschen (mit Bestätigung), und eine automatische Aufbewahrung kann nur die letzten 7, 30, 90, 180 oder 365 Tage behalten. Selbst importierte Protokolle werden nie automatisch gelöscht. Eine zweite Einstellung begrenzt die Anzahl behaltener Sitzungen, und die Listenspalte lässt sich breiter ziehen.',
              'Um mehrere Sitzungen auf einmal loszuwerden, blendet die Schaltfläche Auswählen… bei jedem Eintrag ein Kästchen ein: Häkchen setzen, dann entfernt Löschen den ganzen Stapel nach einer einzigen Bestätigung. Am Computer fügt Strg (⌘) + Klick einzelne Sitzungen hinzu, Umschalt + Klick wählt einen ganzen Bereich.',
        ...
        { id: 'logopts', t: 'Protokoll-Optionen',
          b: ['Unter Erweiterte Optionen \u2192 Log-Nachrichten kannst du die Protokollierung ein- oder ausschalten und das Schreibintervall wählen, mit den drei Einstellungen des Desktop-Clients: nach jeder Aktion, nach jeder Runde (Standard) oder nach jedem Spiel. Eine weitere Option schreibt die .pdb-Datei in einen Ordner deiner Wahl und hält sie in diesem Intervall aktuell, dazu ein letztes Mal beim Verlassen der Seite, damit ein anderes Programm dem Spiel live folgen kann.'],
          note: 'Das Schreiben in einen lokalen Ordner benötigt die File System Access API: nur Chrome, Edge und Opera am Desktop. Sonst erklärt sich die Option selbst und der manuelle Export aus dem Logs-Fenster bleibt verfügbar. Ein Browser kann eine Datei nur ersetzen, nie anhängen \u2014 ein Programm, das die .pdb liest, sollte sie nach jeder Änderung erneut öffnen.' },
        ...
        { id: 'hud', t: 'Stats-HUD an den Sitzen',
          b: ['Das HUD heftet neben jeden Spielersitz ein kleines Statistikfeld, gebaut aus den Händen, die du in deinen Protokollen aufgezeichnet hast: Anzahl beobachteter Hände, dann VPIP (wie oft er pre-flop freiwillig Geld einsetzt), PFR (Pre-flop-Raises) und AF (Aggressionsfaktor), farblich von passiv bis aggressiv abgestuft. Darunter fasst ein Abzeichen den Spieler in Worten zusammen \u2014 Tight-Passiv, Loose-Aggressiv und so weiter \u2014 neben einer kleinen Skala, deren leuchtendes Viertel sich von links nach rechts als tight bis loose und von unten nach oben als passiv bis aggressiv liest. Das Abzeichen erscheint ab der allerersten Hand, bleibt aber bis 25 Hände abgeblendet, ab denen es verlässlich wird. Tippe auf ein Feld für ein ausführliches Popover mit allen Zahlen (3-Bet, Continuation Bet, Fold auf 3-Bet, Steal-Versuche, Showdown-Raten\u2026), und zieh es beiseite, wenn es etwas verdeckt.',
              'Das HUD kennt nur, was du an deinen eigenen Tischen gesehen hast — es liest deine lokalen Hand-Protokolle, die Protokollierung muss also aktiv sein, und die Zahlen werden erst nach genügend Händen aussagekräftig. Standardmäßig ist es aus: Aktiviere es in Erweiterte Optionen \u2192 Assistenz.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'music', t: 'Musikspieler',
          b: ['Der Musik-Eintrag in den Header-Menüs öffnet einen kleinen Lounge-Musikspieler: Wähle einen Titel aus der Playlist, Wiedergabe/Pause, vorheriger/nächster, Zufall und Wiederholung eines Titels, der ganzen Playlist oder gar nichts. Lautstärke, gewählter Titel und Wiederholmodus werden gemerkt. Die Wiedergabe startet nie von selbst — Browser verlangen ein Antippen — und der Spieler ist völlig unabhängig von den Soundeffekten des Spiels.', 'Zwei Daumen unter dem Titel sagen, ob dir gefällt, was läuft. Eine anonyme Stimme pro Gerät, Radios eingeschlossen, jederzeit änderbar oder zurücknehmbar; solange der Betreiber die Summen nicht freigibt, siehst du nur deinen eigenen Daumen.', "Auf iPhone und iPad nutzt der Player standardmäßig die einfache Wiedergabe, damit die Musik mit CarPlay, Bluetooth oder gesperrtem Bildschirm weiterläuft; die Lautstärke wird dann über die Tasten des Geräts oder des Autos geregelt. Die Option „Lautstärke in der App“ bringt Lautstärkeregler, Balance und VU-Meter zurück, der Ton kann im Auto aber stottern."] },
    ...
    },
    {
      ...
      sections: [
        { id: 'where', t: 'Wo die Optionen wohnen',
          b: ['Die Erweiterten Optionen öffnen sich über den Zahnrad-Eintrag jedes Header-Menüs. Sie sind wie im Desktop-Client gruppiert: Benutzeroberfläche, Stil, Sound, Lokales Spiel, Netzwerkspiel, Internetspiel, Spitznamen / Avatare, Log-Meldungen und Sicherung & Zurücksetzen. Jede webspezifische Funktion hat dort ihren eigenen Schalter, damit du alles abschalten kannst, was du nicht nutzt.'] },
        { id: 'cfgxml', t: 'Einstellungen mit dem Desktop-Client austauschen',
          b: ['Deine Einstellungen können zwischen Clients reisen: Die Kategorie Sicherung & Zurücksetzen bietet Export/Import der offiziellen config.xml (die \u007e/.pokerth/config.xml der Desktop- und QML-Clients). Der Export schreibt die gemeinsamen Einstellungen — Name, Anzeigeoptionen, Sounds, Tischvorlieben, Blinds, Stile — und der Import wendet hier eine Desktop-Datei an. Einstellungen, die dieser Client nicht kennt, bleiben in der Datei unangetastet erhalten.', 'Deine Spielernotizen reisen ebenfalls mit der Datei — der Text und die Sternebewertung, so geschrieben, wie die Desktop-Clients sie lesen. Die Farbetiketten bleiben in diesem Client: Das offizielle Format hat kein Feld dafür, ein Import rührt deine also nie an.'] },
        ...
        { id: 'fkeys', t: 'Offizielle Tastenkürzel',
          b: ['Die offiziellen PokerTH-Funktionstasten wirken im Spiel \u2014 Alt+S wirkt \u00fcberall:'],
          keys: [
