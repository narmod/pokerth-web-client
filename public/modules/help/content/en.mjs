  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'language', t: 'Language',
          b: ['The interface is available in 70 languages. Change it any time in Advanced options (gear menu) under User interface. Poker action terms (Fold, Check, Call, Bet, Raise, All-In) stay in English by convention, exactly like the desktop client.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'zoom', t: 'Table zoom (phones)',
          b: ['On small screens, magnifier buttons zoom the table (2\u00d7) and you can pan with a finger \u2014 only the action bar stays fixed; your own box is magnified too, and the view comes back to it when it is your turn. The view follows the active seat automatically and zooms back out at showdown for the overview. This can be turned off in Advanced options. On your turn, if the community cards are out of view, a small copy of them appears at the top of the table; tap it to jump to the cards and back.'],
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'hud', t: 'Stats HUD on the seats',
          b: ['The HUD attaches a small statistics box next to each player\u2019s seat, built from the hands you have recorded in your logs: number of hands observed, then VPIP (how often they voluntarily put money in pre-flop), PFR (pre-flop raises) and AF (aggression factor), color-coded from passive to aggressive. Under them a badge sums the player up in plain words \u2014 Tight-Passive, Loose-Aggressive and so on \u2014 next to a small dial whose lit quadrant reads left to right for tight to loose, and bottom to top for passive to aggressive. The badge shows from the very first hand but stays dimmed until 25 hands, where it becomes reliable. Tap a box for a detailed popover with the full set of numbers (3-bet, continuation bet, fold to 3-bet, steal attempts, showdown rates\u2026), and drag a box to move it if it covers something.',
              'The HUD only knows what you have seen at your own tables \u2014 it reads your local hand logs, so logging must be enabled and the numbers get meaningful after enough hands. It is off by default: enable it in Advanced options \u2192 Assistance.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'music', t: 'Music player',
          b: ['The music entry in the header menus opens a small lounge-music player: pick a track from the playlist, play/pause, previous/next, shuffle, and repeat one track, the whole playlist or nothing. Volume, selected track and repeat mode are remembered. Playback never starts by itself \u2014 browsers require a tap \u2014 and the player is entirely independent from the game sound effects.', 'Two thumbs under the track title say whether you like what is playing. One anonymous vote per device, radios included, and you can change it or take it back at any time; unless the operator reveals the totals, you see only your own thumb.', "On iPhone and iPad the player uses plain playback by default, so the music keeps going with CarPlay, Bluetooth or a locked screen; the volume is then set with the device or car buttons. The “In-app volume” option brings back the volume slider, the balance and the VU meter, but the sound may stutter in the car."] },
    ...
    },
    {
      ...
      sections: [
        { id: 'where', t: 'Where the options live',
          b: ['Advanced options open from the gear entry of any header menu. They are grouped like the desktop client: User interface, Style, Sound, Local game, Network game, Internet game, Nicknames / Avatars, Log messages, and Backup & reset. Every web-specific feature has its own switch there, so you can turn off anything you do not use.'] },
        { id: 'cfgxml', t: 'Exchanging settings with the desktop client',
          b: ['Your settings can travel between clients: the Backup & reset category offers an export/import of the official config.xml file (the \u007e/.pokerth/config.xml used by the desktop and QML clients). Export writes the shared settings \u2014 name, display options, sounds, table preferences, blinds, styles \u2014 and import applies a desktop file here. Settings this client does not know are preserved in the file untouched.', 'Your player notes travel with the file as well — the text and the star rating, written the way the desktop clients read them. The colour labels stay in this client: the official format has no field for them, so an import never touches yours.'] },
