  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'language', t: 'Nyelv',
          b: ['A felület 36 nyelven érhető el. Bármikor átválthatod a Speciális beállításokban (fogaskerék menü), a Felhasználói felület kategóriában. A póker akciókifejezései (Fold, Check, Call, Bet, Raise, All-In) konvenció szerint angolul maradnak, pontosan úgy, mint az asztali kliensben.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'logopts', t: 'Naplóbeállítások',
          b: ['A Speciális beállítások \u2192 Naplóüzenetek alatt be- vagy kikapcsolhatod a rögzítést, és kiválaszthatod az írási időközt (minden akció után, vagy leosztásonként egyszer), akár az asztali kliens beállításaiban. Egy további beállítás a .pdb fájlt közvetlenül egy általad választott mappába írja, és minden leosztás után frissíti — pontosan úgy, ahogy az asztali kliens teszi, hogy más eszközök élőben olvashassák.'],
          note: 'A helyi mappába íráshoz a File System Access API kell: csak asztali Chrome és Edge. A Firefox, a Safari és a mobilböngészők nem képesek rá — a beállítás ilyenkor rövid magyarázatot mutat, a Naplók ablakból való kézi exportálás pedig mindenhol elérhető marad.' },
        ...
        { id: 'hud', t: 'Statisztika-HUD az üléseknél (béta)',
          b: ['A HUD minden játékos ülése mellé egy kis statisztikadobozt tűz, amely a naplóidban rögzített leosztásokból épül: megfigyelt leosztások száma, majd VPIP (milyen gyakran tesz be önként pénzt pre-flop), PFR (pre-flop emelések), AF (agressziófaktor), 3B (3-bet), CB (continuation bet) és F3B (fold 3-betre), passzívtól agresszívig terjedő színkódolással. Érints meg egy dobozt a részletes felugróhoz további számokkal (steal-kísérletek, fold stealre, showdown-arányok\u2026), és húzd arrébb, ha eltakar valamit.',
              'A HUD csak azt tudja, amit a saját asztalaidnál láttál — a helyi leosztásnaplóidat olvassa, ezért a rögzítésnek bekapcsolva kell lennie, és a számok csak elég leosztás után válnak értelmessé. Béta funkció, alapból kikapcsolva: kapcsold be a Speciális beállítások \u2192 Segéd alatt.'] },
      ...
      sections: [
        ...
        { id: 'cups', t: 'A közösségi kupák: BBC és WeCup',
          b: ['Két közösség rendezi a saját versenyeit a pokerth.neten, mindegyik saját webhellyel és saját ranglistával. A Best Brainies Cup (BBC) 2013-ban született lépcsős torna: Step 1-től Step 4-ig lehet feljutni, és minden Step 4-es játék után, a kupa átadásakor új szezon indul. A WeCupnak (WEC) saját, sokkal szélesebbre húzott skálája van — az első helyért 75 pont, majd 45, 30, 20… — és a score-ja a többi taghoz mért játékszámod alapján normalizálja az átlagodat.',
              'Mindkét ranglista a kupa gombbal nyílik, a PokerTH-ranglista mellett. E versenyek asztalbeállításai előbeállításként ott vannak a játék létrehozásakor (BBC Step 1-től 4-ig, WEC és WEC Grand Final), így ugyanolyan körülmények között gyakorolhatsz. A részvételhez az adott kupa webhelyén kell regisztrálni.'],
