  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'language', t: 'Idioma',
          b: ['A interface está disponível em 36 idiomas. Troque a qualquer momento nas Opções avançadas (menu da engrenagem), categoria Interface do usuário. Os termos de ação do pôquer (Fold, Check, Call, Bet, Raise, All-In) permanecem em inglês por convenção, exatamente como no cliente de desktop.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'logopts', t: 'Opções de registro',
          b: ['Em Opções avançadas \u2192 Mensagens de log você pode ligar ou desligar o registro e escolher o intervalo de gravação (após cada ação, ou uma vez por mão), como nas configurações do cliente de desktop. Uma opção extra grava o arquivo .pdb direto numa pasta da sua escolha e o atualiza após cada mão — exatamente como o cliente de desktop faz, para que outras ferramentas o leiam ao vivo.'],
          note: 'Gravar numa pasta local exige a API File System Access: só Chrome e Edge de desktop. Firefox, Safari e navegadores móveis não conseguem — a opção mostra então uma breve explicação, e a exportação manual pela janela Logs continua disponível em todo lugar.' },
        ...
        { id: 'hud', t: 'HUD de estatísticas nos assentos (beta)',
          b: ['O HUD prende uma caixinha de estatísticas ao lado do assento de cada jogador, construída com as mãos gravadas nos seus registros: número de mãos observadas, depois VPIP (com que frequência ele coloca dinheiro voluntariamente pre-flop), PFR (aumentos pre-flop), AF (fator de agressividade), 3B (3-bet), CB (continuation bet) e F3B (fold ao 3-bet), com código de cores do passivo ao agressivo. Toque numa caixa para um popover detalhado com mais números (tentativas de roubo, fold ao roubo, taxas de showdown\u2026), e arraste-a se ela cobrir algo.',
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'translate', t: 'Entender todo mundo',
          b: ['Com a tradução do chat ligada, cada mensagem ganha um botão de tradução que a mostra no seu idioma, pelo tradutor do navegador. As abreviações comuns de mesa (gg, nh, utg\u2026) são explicadas num tooltip ao passar o mouse — as duas opções ficam em Opções avançadas \u2192 Chat.'],
      ...
      sections: [
        ...
        { id: 'cups', t: 'As copas da comunidade: BBC e WeCup',
          b: ['Duas comunidades organizam suas próprias competições no pokerth.net, cada uma com seu site e sua classificação. A Best Brainies Cup (BBC) é um torneio por etapas nascido em 2013: você avança do Step 1 ao Step 4, e uma nova temporada começa depois de cada partida de Step 4, quando a copa é entregue. A WeCup (WEC) tem sua própria tabela, bem mais espalhada — 75 pontos para o primeiro lugar, depois 45, 30, 20… — e seu score normaliza sua média conforme o número de partidas que você jogou em comparação com os demais membros.',
              'As duas classificações abrem pelo botão do troféu, ao lado da classificação do PokerTH. Os ajustes de mesa dessas competições vêm como predefinições ao criar uma partida (BBC Step 1 a 4, WEC e WEC Grand Final), então dá para treinar nas mesmas condições. Participar exige cadastro no site da copa em questão.'],
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'webkeys', t: 'Teclas de letra da web',
          b: ['Extensão web: teclas de uma letra só também disparam as ações e podem ser reatribuídas em Opções avançadas \u2192 Atalhos de teclado:'],
          keys: [
            ['F', 'Fold'],
            ['C', 'Check / Call'],
            ['R', 'Raise'],
