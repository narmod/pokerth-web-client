  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'famboard', t: 'Ranking da família',
          b: ['Somente em servidores privados e partidas LAN, o cliente mantém estatísticas acumuladas por apelido — mãos e partidas jogadas e vencidas, maior ganho, melhor sequência — e as compartilha pelo servidor, para que cada dispositivo na mesa veja o mesmo ranking. Partidas do pokerth.net nunca são registradas assim, e as estatísticas do modo treino ficam completamente separadas.', 'Nessas partidas, o botão do troféu abre a janela de classificações na aba LAN: todos os jogadores, ordenáveis por vários critérios.'] },
        { id: 'language', t: 'Idioma',
          b: ['A interface está disponível em 72 idiomas. Troque a qualquer momento nas Opções avançadas (menu da engrenagem), categoria Interface do usuário. Os termos de ação do pôquer (Fold, Check, Call, Bet, Raise, All-In) permanecem em inglês por convenção, exatamente como no cliente de desktop.'] },
        ...
        { id: 'avatar', t: 'Apelido e avatar',
          b: ['Escolha seu apelido e avatar na tela de conexão antes de conectar. No pokerth.net, seu apelido é o nome da sua conta; os avatares são compartilhados com os outros jogadores pelo servidor de avatares.', 'Seu avatar é enviado ao conectar, e todos os jogadores veem esse mesmo. Se você o trocar enquanto estiver conectado, o novo avatar vale a partir da próxima conexão. A inicial (Aa) não é enviada: os outros jogadores veem o avatar padrão.'] }
      ]
    },
    {
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'zoom', t: 'Zoom da mesa (celulares)',
          b: ['Em telas pequenas, botões de lupa ampliam a mesa (2\u00d7) e você a desloca com o dedo — só a barra de ações fica fixa; a sua caixa também é ampliada e a vista volta para ela quando é a sua vez. A vista segue automaticamente o assento ativo e se afasta no showdown para a visão geral. Desativável nas Opções avançadas. Na sua vez, se as cartas comunitárias estiverem fora da vista, uma pequena cópia aparece no topo da mesa; toque nela para ir até as cartas e voltar.'],
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'hud', t: 'HUD de estatísticas nos assentos',
          b: ['O HUD anexa uma pequena caixa de estatísticas ao lado do lugar de cada jogador, construída a partir das mãos que você registrou nos seus registros: número de mãos observadas, depois VPIP (com que frequência ele coloca dinheiro voluntariamente pre-flop), PFR (aumentos pre-flop) e AF (fator de agressividade), com um código de cores do passivo ao agressivo. Abaixo, um emblema resume o jogador por extenso \u2014 Fechado-Passivo, Aberto-Agressivo, etc. \u2014 ao lado de um pequeno quadrante cujo setor aceso se lê da esquerda para a direita do fechado ao aberto, e de baixo para cima do passivo ao agressivo. O emblema aparece já na primeira mão, mas fica esmaecido até 25 mãos, limiar a partir do qual se torna confiável. Toque em uma caixa para um popover detalhado com todos os números (3-bet, continuation bet, fold ao 3-bet, tentativas de roubo, taxas de showdown\u2026), e arraste-a se ela cobrir alguma coisa.',
              'O HUD só conhece o que você viu nas suas próprias mesas — ele lê seus registros locais de mãos, então o registro precisa estar ligado e os números ganham sentido depois de mãos suficientes. Ele vem desligado por padrão: ligue-o em Opções avançadas \u2192 Assistência.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'privatemsg', t: "Mensagens privadas",
          b: ["Escreva para um único jogador sem que todo o lobby leia. O envelope ao lado de um nome na lista de jogadores abre uma conversa com ele; o envelope no cabeçalho do lobby reabre a última. As conversas ficam guardadas neste dispositivo e continuam lá quando você volta, então um chat retomado dias depois mantém seu histórico — um contador vermelho no envelope mostra o que você ainda não leu, e a lixeira no título da janela apaga uma conversa definitivamente."],
          keys: [
            ["/msg <apelido> <mensagem>", "Enviar uma mensagem privada a partir do chat do lobby"],
            ["/msg \"<apelido>\" <mensagem>", "O mesmo, quando o apelido contém espaços"]],
          note: "As mensagens são limitadas a 128 caracteres. O servidor não entrega uma mensagem privada a um jogador sentado em uma mesa em andamento, e o histórico fica armazenado apenas neste navegador — ele não acompanha você em outro dispositivo." },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'music', t: 'Reprodutor de música',
          b: ['A entrada de música dos menus do cabeçalho abre um pequeno reprodutor de música ambiente: escolha uma faixa da playlist, tocar/pausar, anterior/próxima, aleatório, e repetição de uma faixa, da playlist inteira ou de nada. O volume, a faixa escolhida e o modo de repetição ficam memorizados. A reprodução nunca começa sozinha — os navegadores exigem um toque — e o reprodutor é totalmente independente dos efeitos sonoros do jogo.', 'Dois polegares abaixo do título da faixa dizem se você gosta do que está tocando. Um voto anônimo por dispositivo, rádios incluídas, e você pode mudá-lo ou retirá-lo a qualquer momento; enquanto o operador não revelar os totais, você vê apenas o seu próprio polegar.', "No iPhone e no iPad o player usa por padrão uma reprodução simples, para que a música continue com CarPlay, Bluetooth ou tela bloqueada; o volume é então ajustado pelos botões do aparelho ou do carro. A opção “Volume no app” traz de volta o controle de volume, o balanço e o VU meter, mas o som pode engasgar no carro."] },
    ...
    },
    {
      ...
      sections: [
        { id: 'where', t: 'Onde as opções moram',
          b: ['As Opções avançadas abrem pela entrada da engrenagem de qualquer menu do cabeçalho. Estão agrupadas como no cliente de desktop: Interface do usuário, Estilo, Som, Partida local, Partida em rede, Partida pela Internet, Apelidos / Avatares, Mensagens de log, e Backup e redefinição. Cada recurso específico da web tem lá seu próprio interruptor, para você desligar tudo o que não usa.'] },
        { id: 'cfgxml', t: 'Trocar configurações com o cliente de desktop',
          b: ['Suas configurações podem viajar entre clientes: a categoria Backup e redefinição oferece exportar/importar o arquivo config.xml oficial (o \u007e/.pokerth/config.xml usado pelos clientes de desktop e QML). A exportação grava as configurações compartilhadas — nome, opções de exibição, sons, preferências de mesa, blinds, estilos — e a importação aplica aqui um arquivo do desktop. As configurações que este cliente não conhece ficam intactas no arquivo.', 'Suas notas de jogador também viajam com o arquivo: o texto e a avaliação em estrelas, escritos do jeito que os clientes de desktop leem. As etiquetas de cor ficam neste cliente: o formato oficial não tem campo para elas, então uma importação nunca as toca.'] },
