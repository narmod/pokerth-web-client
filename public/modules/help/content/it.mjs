  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'famboard', t: 'Classifica di famiglia',
          b: ['Solo sui server privati e nelle partite LAN, il client conserva statistiche cumulative per nickname — mani e partite giocate e vinte, vincita più grande, miglior serie — e le condivide tramite il server, così ogni dispositivo attorno al tavolo vede la stessa classifica. Le partite su pokerth.net non vengono mai tracciate in questo modo, e le statistiche della modalità allenamento restano completamente separate.', 'In queste partite, il pulsante del trofeo apre la finestra delle classifiche sulla scheda LAN: tutti i giocatori, ordinabili secondo diversi criteri.'] },
        { id: 'language', t: 'Lingua',
          b: ['L\u2019interfaccia è disponibile in 75 lingue. Cambiala in qualsiasi momento nelle Opzioni avanzate (menu con l\u2019ingranaggio), categoria Interfaccia utente. I termini d\u2019azione del poker (Fold, Check, Call, Bet, Raise, All-In) restano in inglese per convenzione, esattamente come nel client desktop.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'zoom', t: 'Zoom del tavolo (telefoni)',
          b: ['Su schermi piccoli, i pulsanti lente ingrandiscono il tavolo (2\u00d7) e puoi trascinarlo col dito — solo la barra delle azioni resta fissa; anche il tuo riquadro viene ingrandito e la vista ci torna quando è il tuo turno. La vista segue automaticamente il posto attivo e si allontana allo showdown per la panoramica. Disattivabile nelle Opzioni avanzate. Al tuo turno, se le carte comuni sono fuori dalla vista, ne appare una piccola copia in alto sul tavolo; toccala per andare alle carte e tornare indietro.'],
      ...
      sections: [
        ...
        { id: 'journal', t: 'Registri delle mani e la finestra «Logs»',
          b: ['Oltre alla cronologia dal vivo, ogni mano che giochi viene registrata localmente nel browser, nello stesso formato dei file di log .pdb del client ufficiale. La finestra Logs (Opzioni avanzate \u2192 Messaggi di log \u2192 Gestisci i log\u2026) elenca le tue sessioni e ti permette di lavorarci: vedere l\u2019anteprima di una sessione con ricerca ed evidenziazione, filtrare per partita, esportare in HTML o testo semplice, salvare il file .pdb grezzo, o importare un .pdb registrato dal client desktop. Le sessioni si eliminano una alla volta o tutte insieme (con conferma), e un\u2019impostazione di conservazione automatica può tenere solo gli ultimi 7, 30, 90, 180 o 365 giorni. I log che importi tu non vengono mai eliminati automaticamente. Una seconda impostazione limita quante sessioni vengono conservate e la colonna dell’elenco si può allargare trascinandola.',
              'Per fare pulizia in una sola volta, il pulsante Seleziona… mostra una casella su ogni voce: spunta quelle da eliminare e Elimina cancella l’intero gruppo dopo una sola conferma. Su computer, Ctrl (⌘) + clic aggiunge le sessioni una alla volta e Maiusc + clic ne prende un intero intervallo.',
        ...
        { id: 'logopts', t: 'Opzioni di registrazione',
          b: ['In Opzioni avanzate \u2192 Messaggi di log puoi attivare o disattivare la registrazione e scegliere l\u2019intervallo di scrittura, con le stesse tre impostazioni del client desktop: dopo ogni azione, dopo ogni mano (predefinito) o dopo ogni partita. Un\u2019altra opzione scrive il file .pdb in una cartella a tua scelta e lo mantiene aggiornato con quell\u2019intervallo, più un\u2019ultima volta quando lasci la pagina, così un altro strumento può seguire la partita in diretta.'],
          note: 'Scrivere in una cartella locale richiede la File System Access API: solo Chrome, Edge e Opera desktop. Altrove l\u2019opzione si spiega da sé e l\u2019esportazione manuale dalla finestra dei log resta disponibile. Un browser può solo sostituire un file, mai accodare, quindi uno strumento che legge il .pdb dovrebbe riaprirlo dopo ogni modifica.' },
        ...
        { id: 'hud', t: 'HUD di statistiche sui posti',
          b: ['L\u2019HUD attacca una piccola casella di statistiche accanto al posto di ogni giocatore, costruita dalle mani che hai registrato nei tuoi log: numero di mani osservate, poi VPIP (quanto spesso mette volontariamente soldi pre-flop), PFR (rilanci pre-flop) e AF (fattore di aggressività), con un codice colore dal passivo all\u2019aggressivo. Sotto, un distintivo riassume il giocatore a parole \u2014 Stretto-Passivo, Largo-Aggressivo e così via \u2014 accanto a un piccolo quadrante il cui settore acceso si legge da sinistra a destra da stretto a largo, e dal basso verso l\u2019alto da passivo ad aggressivo. Il distintivo compare fin dalla prima mano ma resta attenuato fino a 25 mani, soglia oltre la quale diventa affidabile. Tocca una casella per un popover dettagliato con tutti i numeri (3-bet, continuation bet, fold al 3-bet, tentativi di furto, tassi di showdown\u2026), e trascinala se copre qualcosa.',
              'L\u2019HUD conosce solo ciò che hai visto ai tuoi tavoli — legge i tuoi log locali delle mani, quindi la registrazione deve essere attiva e i numeri diventano significativi dopo abbastanza mani. È disattivata di default: attivala in Opzioni avanzate \u2192 Assistenza.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'music', t: 'Lettore musicale',
          b: ['La voce musica dei menu dell\u2019intestazione apre un piccolo lettore di musica d\u2019ambiente: scegli un brano dalla playlist, riproduci/pausa, precedente/successivo, casuale, e ripetizione di un brano, di tutta la playlist o di niente. Volume, brano selezionato e modalità di ripetizione vengono ricordati. La riproduzione non parte mai da sola — i browser richiedono un tocco — e il lettore è del tutto indipendente dagli effetti sonori del gioco.', 'Due pollici sotto il titolo del brano dicono se ti piace ciò che sta suonando. Un voto anonimo per dispositivo, radio comprese, e puoi cambiarlo o ritirarlo in qualsiasi momento; finché l\u2019operatore non mostra i totali, vedi solo il tuo pollice.', "Su iPhone e iPad il lettore usa per impostazione predefinita una riproduzione semplice, così la musica continua con CarPlay, Bluetooth o a schermo bloccato; il volume si regola allora con i tasti del dispositivo o dell’auto. L’opzione «Volume nell’app» ripristina il cursore del volume, il bilanciamento e il VU meter, ma in auto l’audio può andare a scatti."] },
    ...
    },
    {
      ...
      sections: [
        { id: 'where', t: 'Dove vivono le opzioni',
          b: ['Le Opzioni avanzate si aprono dalla voce con l\u2019ingranaggio di qualsiasi menu dell\u2019intestazione. Sono raggruppate come nel client desktop: Interfaccia utente, Stile, Suono, Partita locale, Partita in rete, Partita Internet, Nickname / Avatar, Messaggi di log, e Backup e ripristino. Ogni funzione specifica del web ha lì il suo interruttore, così puoi disattivare tutto ciò che non usi.'] },
        { id: 'cfgxml', t: 'Scambiare le impostazioni con il client desktop',
          b: ['Le tue impostazioni possono viaggiare tra i client: la categoria Backup e ripristino offre esportazione/importazione del file config.xml ufficiale (il \u007e/.pokerth/config.xml usato dai client desktop e QML). L\u2019esportazione scrive le impostazioni condivise — nome, opzioni di visualizzazione, suoni, preferenze del tavolo, bui, stili — e l\u2019importazione applica qui un file del desktop. Le impostazioni che questo client non conosce restano intatte nel file.', 'Anche le tue note sui giocatori viaggiano con il file: il testo e la valutazione a stelle, scritti come li leggono i client desktop. Le etichette colorate restano in questo client: il formato ufficiale non ha un campo per loro, quindi un\u2019importazione non le tocca mai.'] },
        ...
        { id: 'fkeys', t: 'Scorciatoie da tastiera ufficiali',
          b: ['I tasti funzione ufficiali di PokerTH funzionano durante una partita \u2014 Alt+S funziona ovunque:'],
          keys: [
