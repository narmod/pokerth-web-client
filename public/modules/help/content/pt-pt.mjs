  ...
  chapters: [
    {
      ...
      sections: [
        ...
        { id: 'famboard', t: 'Classificação da família',
          b: ['Apenas em servidores privados e partidas LAN, o cliente guarda estatísticas acumuladas por alcunha — mãos e partidas jogadas e ganhas, maior ganho, melhor série — e partilha-as através do servidor, para que cada dispositivo à mesa veja a mesma classificação. As partidas de pokerth.net nunca são registadas desta forma, e as estatísticas do modo treino ficam completamente separadas.', 'Nestes jogos, o botão do troféu abre a janela de classificações no separador LAN: todos os jogadores, ordenáveis por vários critérios.'] },
        { id: 'language', t: 'Idioma',
          b: ['A interface está disponível em 77 idiomas. Muda-o a qualquer momento nas Opções avançadas (menu da engrenagem), categoria Interface do utilizador. Os termos de ação do póquer (Fold, Check, Call, Bet, Raise, All-In) mantêm-se em inglês por convenção, exatamente como no cliente de secretária.'] },
        ...
        { id: 'avatar', t: 'Alcunha e avatar',
          b: ['Escolhe a tua alcunha e o teu avatar no ecrã de ligação antes de te ligares. Em pokerth.net, a tua alcunha é o nome da tua conta; os avatares são partilhados com os outros jogadores através do servidor de avatares.', 'O teu avatar é enviado quando te ligas e todos os jogadores veem esse mesmo. Se o mudares enquanto estás ligado, o novo avatar aplica-se a partir da próxima ligação. A inicial (Aa) não é enviada: os outros jogadores veem o avatar predefinido.'] }
      ]
    },
    {
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'zoom', t: 'Zoom da mesa (telemóveis)',
          b: ['Em ecrãs pequenos, botões de lupa ampliam a mesa (2\u00d7) e podes deslocá-la com o dedo — só a barra de ações fica fixa; a tua caixa também é ampliada e a vista volta a ela quando é a tua vez. A vista segue automaticamente o lugar ativo e afasta-se no showdown para a visão geral. Desativável nas Opções avançadas. Na tua vez, se as cartas comunitárias estiverem fora da vista, aparece uma pequena cópia no topo da mesa; toca-lhe para ir às cartas e voltar.'],
      ...
      sections: [
        ...
        { id: 'journal', t: 'Registos de mãos e a janela \u201cRegistos\u201d',
          b: ['Além do histórico em direto, cada mão que jogas é gravada localmente no navegador, no mesmo formato dos ficheiros de registo .pdb do cliente oficial. A janela Registos (Opções avançadas \u2192 Mensagens de registo \u2192 Gerir registos\u2026) lista as tuas sessões e deixa-te trabalhar com elas: pré-visualizar uma sessão com pesquisa e realce, filtrar por partida, exportar em HTML ou texto simples, guardar o ficheiro .pdb em bruto, ou importar um .pdb gravado pelo cliente de secretária. As sessões apagam-se uma a uma ou todas de uma vez (com confirmação), e uma retenção automática pode manter só os últimos 7, 30, 90, 180 ou 365 dias. Os registos que importa nunca são removidos automaticamente. Um segundo ajuste limita quantas sessões são mantidas, e a coluna da lista pode ser alargada arrastando.',
              'Para limpar várias sessões de uma vez, o botão Selecionar… mostra uma caixa em cada entrada: marque as que já não quer e Eliminar apaga todo o lote após uma única confirmação. No computador, Ctrl (⌘) + clique acrescenta sessões uma a uma e Shift + clique abrange um intervalo inteiro.',
        ...
        { id: 'logopts', t: 'Opções de registo',
          b: ['Nas Opções avançadas \u2192 Mensagens de registo podes ligar ou desligar o registo e escolher o intervalo de escrita, com as mesmas três definições do cliente de secretária: após cada ação, após cada mão (predefinição) ou após cada jogo. Outra opção escreve o ficheiro .pdb numa pasta à tua escolha e mantém-no atualizado nesse intervalo, mais uma vez quando sais da página, para que outra ferramenta possa acompanhar o jogo em direto.'],
          note: 'Escrever numa pasta local precisa da API File System Access: apenas Chrome, Edge e Opera de secretária. Nos restantes a opção explica-se e a exportação manual a partir da janela de registos continua disponível. Um navegador só pode substituir um ficheiro, nunca acrescentar, por isso uma ferramenta que leia o .pdb deve reabri-lo após cada alteração.' },
        ...
        { id: 'hud', t: 'HUD de estatísticas nos lugares',
          b: ['O HUD anexa uma pequena caixa de estatísticas ao lado do lugar de cada jogador, construída a partir das mãos que registaste nos teus registos: número de mãos observadas, depois VPIP (com que frequência põe dinheiro voluntariamente pre-flop), PFR (subidas pre-flop) e AF (fator de agressividade), com um código de cores do passivo ao agressivo. Por baixo, um emblema resume o jogador por extenso \u2014 Fechado-Passivo, Aberto-Agressivo, etc. \u2014 ao lado de um pequeno quadrante cujo setor aceso se lê da esquerda para a direita do fechado ao aberto, e de baixo para cima do passivo ao agressivo. O emblema aparece logo na primeira mão mas fica esbatido até às 25 mãos, limiar a partir do qual se torna fiável. Toca numa caixa para um popover detalhado com todos os números (3-bet, continuation bet, fold ao 3-bet, tentativas de roubo, taxas de showdown\u2026), e arrasta-a se tapar alguma coisa.',
              'O HUD só conhece o que viste nas tuas próprias mesas — lê os teus registos locais de mãos, pelo que o registo tem de estar ligado e os números ganham sentido depois de mãos suficientes. Está desligada por predefinição: liga-a em Opções avançadas \u2192 Assistência.'] },
      ...
      sections: [
        ...
        { id: 'forumnews', t: 'Novidades do fórum no lobby',
          b: ['O botão de jornal no cabeçalho do lobby abre as últimas mensagens do fórum pokerth.net, uma entrada por tópico, cada fórum com a sua cor. O emblema no botão conta as mensagens por ler; abrir uma mensagem (novo separador) marca-a como lida, e “Marcar tudo como lido” limpa tudo de uma vez.',
              ...
              'O separador “Eventos” mostra os próximos jogos da BBC e a próxima Monthly Cup com o número de jogadores inscritos, e os últimos vencedores da BBC, da WEC e da Monthly Cup. As horas estão na sua hora local, e um toque abre o site da comunidade. A opção “Mostrar conteúdo da comunidade (BBC / WEC)” oculta este separador.'] },
    ...
    },
    {
      ...
      sections: [
        ...
        { id: 'music', t: 'Leitor de música',
          b: ['A entrada de música dos menus do cabeçalho abre um pequeno leitor de música ambiente: escolhe uma faixa da lista, reproduzir/pausar, anterior/seguinte, aleatório, e repetição de uma faixa, de toda a lista ou de nada. O volume, a faixa escolhida e o modo de repetição ficam memorizados. A reprodução nunca começa sozinha — os navegadores exigem um toque — e o leitor é totalmente independente dos efeitos sonoros do jogo.', 'Dois polegares por baixo do título da faixa dizem se gostas do que está a tocar. Um voto anónimo por dispositivo, rádios incluídas, e podes alterá-lo ou retirá-lo a qualquer momento; enquanto o operador não revelar os totais, vês apenas o teu polegar.', "No iPhone e no iPad o leitor usa por predefinição uma reprodução simples, para que a música continue com CarPlay, Bluetooth ou ecrã bloqueado; o volume ajusta-se então com os botões do dispositivo ou do carro. A opção «Volume na app» repõe o controlo de volume, o balanço e o VU meter, mas o som pode falhar no carro."] },
    ...
    },
    {
      ...
      sections: [
        { id: 'where', t: 'Onde vivem as opções',
          b: ['As Opções avançadas abrem-se pela entrada da engrenagem de qualquer menu do cabeçalho. Estão agrupadas como no cliente de secretária: Interface do utilizador, Estilo, Som, Partida local, Partida em rede, Partida pela Internet, Alcunhas / Avatares, Mensagens de registo, e Cópia de segurança e reposição. Cada funcionalidade específica da web tem lá o seu próprio interruptor, para desligares tudo o que não usas.'] },
        { id: 'cfgxml', t: 'Trocar definições com o cliente de secretária',
          b: ['As tuas definições podem viajar entre clientes: a categoria Cópia de segurança e reposição oferece exportar/importar o ficheiro config.xml oficial (o \u007e/.pokerth/config.xml usado pelos clientes de secretária e QML). A exportação escreve as definições partilhadas — nome, opções de visualização, sons, preferências de mesa, blinds, estilos — e a importação aplica aqui um ficheiro da secretária. As definições que este cliente não conhece ficam intactas no ficheiro.', 'As tuas notas de jogador também viajam com o ficheiro: o texto e a avaliação em estrelas, escritos tal como os clientes de secretária os leem. As etiquetas de cor ficam neste cliente: o formato oficial não tem campo para elas, por isso uma importação nunca lhes toca.'] },
