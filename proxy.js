...
function importPackage(kind, idHint, zipBuf, cb) {
  ...
  if (kind === 'table') {
    ...
    if (['', 'pokerth', 'chip', 'plate', 'card', 'compact', 'bar', 'onyx-pill'].indexOf(id) >= 0) id = 'seat-' + id;   // never clobber a built-in / official seat id
...
function handleAdmin(req, res, reqPathOnly, query) {
  ...
  if (reqPathOnly === '/admin/pkg-remove' && req.method === 'POST') {
    return readJsonBody(req, function (d) {
      ...
      if (kind === 'seat' && ['', 'pokerth', 'chip', 'plate', 'card', 'compact', 'bar', 'onyx-pill'].indexOf(id) >= 0) return adminJson(res, 400, { ok: false, error: 'built-in seat cannot be removed' });
