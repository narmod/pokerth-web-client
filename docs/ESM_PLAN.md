...
| 2026-07-19 | 0.3.851-beta | 9g-B2 ui/odds-panel.mjs (9 fonctions : calcWinProb Monte Carlo, bandeau force de main _hs* + contraste thème clair, _gipAssistSync, renderPreFlop/HandStrength, renderOddsMonitor + alias _renderOdds ; adaptations cards.mjs importé, _advGet/_oddsSeq → window.* ; corps identiques prouvés) — 22 tests | 660 → 648 Ko |
| 2026-07-19 | 0.3.852-beta | 9g-B3 ui/table-cards.mjs (6 fonctions + _refreshOwnCards : cartes propres/option révéler-au-clic, renderComm, _setCanShow « Show », animations distribution & jeton→pot ; imports deck/odds-panel, _ownReveal→window (4×), ponts renderSeats/renderMyTurnActions [ajouté] ; corps identiques prouvés) — 21 tests | 648 → 644 Ko |
| 2026-07-19 | 0.3.854-beta | 9g-B4 ui/action-bar.mjs (12 fonctions + toggleAssist/setAssist : renderMyTurnActions live/aperçu, doAction/doRaise/confirmCall, _runPreAction, _playAutoMode, notifs de tour, assistance ; 10 imports, ponts logAction/renderGameWaiting/updateBottomLayout/clearSpectatorActions ajoutés, vars top-level → window.* ; ⚠ bug latent `KB` de confirmCall conservé & documenté, fix en push séparé ; corps identiques prouvés) — 32 tests · **fin des vagues B : le noyau restant = handleMsg + renderSeatsImmediate + satellites (#9g-C)** | 648 → 623 Ko |
| 2026-07-19 | 0.3.855-beta | 9g-B4b fix bug latent `KB` : confirmCall relit les bindings via window._keyBindings() → le relabel « Confirm $X ? » de l'anti-call accidentel s'affiche enfin (test étendu, 33 ✓) | 623 Ko |
| 2026-07-19 | 0.3.856-beta | 9g-C1 net/msg-avatars.mjs — PILOTE de l'éclatement de handleMsg : 5 cases (AvatarRequest/Header/Data/End, UnknownAvatar) → handlers onXxx(sub), cases réduites à la délégation `case T.X: { onXxx(sub); break; }` ; équivalence case↔handler prouvée par AST — 12 tests (cycle complet Header→Data→End + upload chunké 256 o) | 620 Ko |
| 2026-07-19 | 0.3.857-beta | 9g-C2 net/msg-social.mjs — 10 cases (kick-petitions ×5, InviteNotify/RejectInvNotify, Chat [interop /emoji conservée à l'octet], TimeoutWarning, ChatReject) → handlers onXxx(sub) ; adaptation addGameChat→window (2×) ; équivalence AST prouvée — 14 tests | 616 Ko |
| 2026-07-19 | 0.3.858-beta | 9g-C3 net/msg-lobby.mjs — 15 cases (Announce/InitAck/AuthChallenge, Error, ReportGameAck, AdminBanPlayerAck, Statistics, PlayerList, PlayerInfoReply, GameList* ×6) → handlers ; règle établie : handleMsg n'ayant rien après son switch, `break` de case → `return;` (générateur ET vérificateur AST, sorties anticipées d'InitAck couvertes) ; 13 imports, 3 ponts ajoutés (_defaultNameForMode/_tableHasPid/renderWaitingPanel), App conservé nu (const lexical global) — 18 tests | 616 → 605 Ko |
| 2026-07-19 | 0.3.859-beta | 9g-C4 net/msg-game-join.mjs — 9 cases (JoinGameAck/Failed, GamePlayerJoined/Left, RemovedFromGame, StartEvent, GameAdminChanged, GameSpectator±) → handlers ; 11 imports, pont setPot ajouté, équivalence AST prouvée — 21 tests (rejoin/pth_resume, arrivée tardive spectateur, StartEventAck, sons) | 605 → 594 Ko |
| 2026-07-19 | 0.3.861-beta | 9g-C5a game/msg-hand.mjs — GameStartInitial + HandStart (454 l : gel des sièges, déchiffrement PTHCrypto, bootstrap spectateur, blinds/toast/minuteur, snapshot des stacks) ; **leçon majeure : acorn-walk ne visite pas les Identifier en position de PATTERN → les affectations nues (_sdLosers/_sdWinners/_ownReveal/_lastCallSeen/_callConfirmArmed) échappaient aux scans et auraient jeté en module strict — audit par TOKENS désormais obligatoire avant chaque push C** ; 2e bug latent conservé & documenté : closure setTimeout de animateDealerMove lit _prevDealerPid après écrasement → anim(nouveau,nouveau) no-op depuis toujours ; équivalence AST prouvée — 17 tests · ⚠ test manuel en ligne obligatoire | 594 → 570 Ko |
| 2026-07-19 | 0.3.862-beta | 9g-C5a-bis fix bug latent animation dealer : from/to capturés à la planification du setTimeout (l'ancienne closure relisait _prevDealerPid après écrasement → no-op) — le puck dealer s'anime enfin entre les mains ; test mis à jour (9→5) 17 ✓ | 570 Ko |
| 2026-07-19 | 0.3.863-beta | 9g-C5b game/msg-hand.mjs (2/2) — les 11 dernières grosses cases (PlayersTurn/ActionDone, DealFlop/Turn/River, AllInShowCards, EndOfHandShow/Hide, AfterHandShowCards, EndOfGame, YourActionRejected) → handlers ; pont logEliminations ajouté ; audit tokens ✓ (0 écriture nue) ; équivalence AST prouvée — 25 tests · **handleMsg est réduit à un dispatcher : 52 cases dont 40 délégations une-ligne** · ⚠ test manuel en ligne obligatoire | 570 → 547 Ko |
| 2026-07-19 | 0.3.864-beta | 9g-C6 game/seat-render.mjs — renderSeatsImmediate (999 l : rotation vue-joueur, avatars/drapeaux/badges, cartes adverses, pucks D-SB-BB, timer, estompage/surbrillance showdown, géométrie QML) + renderSeats (débounce rAF) + getPlayerName/isBot/getPlayerInitial/getPlayerTypeBadge ; leçon : motif de strip des ponts À BACKREFERENCE stricte (le motif lâche avalait 7 vraies instructions window.X = false/true — attrapé par la preuve d'équivalence) ; audit tokens 0 écriture nue ; équivalence 6/6 — 13 tests · ⚠ test manuel (rendu des sièges = cœur visuel) | 547 → 459 Ko |
| 2026-07-19 | 0.3.865-beta | 9g-A1 préparation type=module — inventaire automatisé des consommations externes (HTML inline, chat-emotes.js, modules) : 31 fonctions + App pontés explicitement ; les 8 vars mutables partagées (_sdLosers/_sdWinners/_ownReveal/_lastCallSeen/_callConfirm*/_lastBoardCount/_oddsSeq) deviennent des propriétés window À LA SOURCE (stockage unique avant la bascule) ; comportement inchangé en script classique | 459 Ko |
| 2026-07-19 | 0.3.866-beta | **9g-A2 : pokerth.js est un MODULE** — audit strict complet (parse module OK ; 5 implicites sloppy convertis en window.* : _ipBlockUntil/_rebindAction ×2, _reactMuted/_reactPinned ×4, directWS ×12 + init explicite ; session.mjs aligné) ; tag `<script type=module>` (ordre de défèrement inchangé, dernier du document) ; test-boot évalue désormais en sémantique module simulée (IIFE stricte) 11/11 · ⚠ test manuel complet obligatoire — c'est la bascule la plus risquée du chantier | 459 Ko |
| 2026-07-19 | — | **CHANTIER #9g TERMINÉ — validé en ligne (spectateur pokerth.net, partie complète, console muette).** Bilan : monolithe 954 → 459 Ko (−52 %), handleMsg 2080 l → dispatcher 145 l (40/52 délégations), 20+ modules ESM, pokerth.js évalué en module strict. Suite (opportuniste, sans vague dédiée) : remplacer les ponts window.* par de vrais `import` au fil des retouches ; retirer les diags temporaires [prearm]/[cards-diag] une fois le bug « cartes invisibles en auth » clos. |  |
