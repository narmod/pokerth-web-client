...
- Theme panel fully localised in all 70 languages, with live switching and a live preview
  of each deck.
...
- Internationalisation in 70 languages, auto-detected and switchable on the fly.
...
- **Translating `/faq`** — `/rules` and `/faq` are done in all 70 languages
...
- **Native review of the smaller languages** — the translations for Hindi,
  ...
  Gaelic, Icelandic, Uzbek, Burmese, Georgian, Kazakh, Sinhala, Azerbaijani, Armenian and Belarusian were written without an established poker vocabulary to
