...
with them reproduces its commits byte for byte.
...
- Step 3 content: the `seo-i18n/` entries and the `proxy.js` SEO entries
  (`SEO_I18N`, `OG_LOCALE`, `SEO_BODY_I18N`, `SEO_RULES_I18N`, `SEO_FAQ_I18N`),
  the English "NN languages" phrases of `proxy.js`, and the languages that
  spell the count out.
